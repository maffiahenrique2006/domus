# Consuming Domus Design System in web apps

Read `artifacts/domus-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/domus-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/domus-ds/styles.css";
```
...
`node_modules/@workspace/domus-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/domus-ds/components/ui/button";
import { cn } from "@workspace/domus-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/domus-ds/hooks/use-toast";
```
...
`@workspace/domus-ds/components/ui/button`. Run the app's typecheck
