# Migrating Expo UI to Domus Design System

Read `artifacts/domus-ds/docs/AGENTS.md` and
`artifacts/domus-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/domus-ds/lib/native-theme` and
  `@workspace/domus-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/domus-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/domus-ds` and retains only
