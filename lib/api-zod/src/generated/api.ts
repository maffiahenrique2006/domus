...
export const sendChatMessageBodySessionIdMax = 100;



export const SendChatMessageBody = zod.object({
  "content": zod.string().min(1).max(sendChatMessageBodyContentMax),
  "sessionId": zod.string().max(sendChatMessageBodySessionIdMax).optional().describe('Client-generated identifier for the demo session (used only to group token-usage logs; not persisted with message content).')
})
...
}).describe('Token usage reported by the OpenAI Responses API for a single Domus AI call.')
})
